...
Create a file called `config_local.py` next to `app.py`:
...
# Optional. Blank = gemini-2.5-flash (free tier). Other options:
# gemini-3.6-flash, or gemini-flash-latest to track the newest release.
GEMINI_MODEL   = ""

# Optional. Moves the player registry to MySQL so it survives a container
# rebuild. Blank keeps everything in the local SQLite file. Accepts
# mysql://... or the jdbc:mysql://... string panels usually give you.
MYSQL_URL      = ""
```

Excluded from every zip and in `.gitignore`, so your keys stay on your server.
...
Set `GITHUB_TOKEN` and the bot pulls the latest code from GitHub every time it
starts, so you stop re-uploading zips. Leave it unset to turn the whole thing
...
3. **Expiration** → pick a long one, or "No expiration". The default is 30 days,
   and when it lapses the bot just stops updating — `;version` will say
   "failed", and this is the reason nine times out of ten.
4. **Resource owner** → the account that owns `beybalde-`
5. **Repository access** → "Only select repositories" → tick `beybalde-`
6. **Repository permissions** → **Contents: Read-only**. GitHub adds
   "Metadata: Read-only" by itself and will not let you remove it. That is
   expected — leave it. Contents is the only one you set yourself.
7. **Generate token**, copy the `github_pat_...` string.

Read-only is all it needs: this token only ever downloads. **Never give it write
access** — anything that can push to the repo can run code on your host.

Put it wherever you put `BOT_TOKEN` (`.env`, panel Startup variables, or
`config_local.py`):

```python
GITHUB_TOKEN  = "github_pat_..."

# Optional. Which repo to pull from. Blank uses the repo this code ships from,
# which is almost certainly what you want. Paste the link in whatever form you
# have it — browser URL, clone link, or just owner/repo. All of these work:
...
#
# SET THIS if your code lives on a branch that hasn't been merged into main
...
`.env`, `config_local.py` and everything under `data/` are never touched by an
update, so your keys and player data survive. Turn the updater off entirely with
