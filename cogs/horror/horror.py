"""Horror Story: owner encounters plus rare automatic server-wide encounters."""
...
class TargetOnlyView(discord.ui.View):
    def __init__(self, cog: "HorrorCog", target_id: int, *, timeout: float = PROMPT_TIMEOUT):
        super().__init__(timeout=timeout); self.cog=cog; self.target_id=int(target_id); self.message=None
...
class HorrorChallengeView(TargetOnlyView):
    ...
    async def no(self, interaction, _button):
        ...
        forced=ForcedBattleView(self.cog,self.target_id); forced.message=interaction.message; self.stop()
        await interaction.response.edit_message(content=f"<@{self.target_id}>\n**UNKNOWN:** battle me.",view=forced)

class HorrorCog(commands.Cog, name="Horror Story"):
    def __init__(self, bot):
        self.bot=bot; self._curse_locks={}; self.auto_horror_spawn.start(); self.horror_cleanup.start()
    def cog_unload(self):
        self.auto_horror_spawn.cancel(); self.horror_cleanup.cancel()
    ...
    async def accept_battle(self, interaction, target_id, view):
        ...
        horror_state.save_encounter(target_id,status="battle_requested",battle_started=True,equipped_bey=bey_name,equipped_copy_id=copy_id)
        view.stop(); await interaction.response.edit_message(content=f"<@{target_id}>\n**UNKNOWN:** good.",view=None)
        self.bot.dispatch("horror_battle_requested",interaction.channel,interaction.user,bey_name,copy_id)
